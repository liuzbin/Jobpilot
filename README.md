...
- [ ] 【需要你在自己的 Windows 机器上验证，云端沙盒无法覆盖】按"一、启动本地 App"里的流程，在一台干净的 Windows 环境（未装过 GTK3）上首次启动本地 App，触发一次简历 PDF 生成，确认能看到自动检测/安装 GTK3 的提示和过程，安装完成后 PDF 能正常生成；如果安装后需要重启 App 才能生效，确认提示文案讲清楚了这一点。
